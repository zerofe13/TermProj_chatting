...
CMyProtocol::CMyProtocol(CTermProj24ServerDlg *pDlg, CDataSocket *pSocket)
{
...
void CMyProtocol::ProcessReceive(CDataSocket *pSocket, int nErrorCode)
{
	...
	if (aType == STOP_AND_WAIT)		//STOP AND WAIT ******************************************
		...
		ptcSetPktReceived(&recvBuffer);	//ACK TYPE NOT DECLARED
...
UINT RXThread(LPVOID arg)
{
	...
	CTermProj24ServerDlg *m_pDlg = pArg->m_pDlg;
